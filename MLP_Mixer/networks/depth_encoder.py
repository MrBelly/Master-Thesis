...
class MLPMixer(nn.Module):
    ...
    def __init__(self, in_channels, dim, patch_size, image_size, depth, token_dim, channel_dim):
        ...
        self.conv_up_1 = ConvBlock(in_channels,in_channels)
        self.conv_up_2 = ConvBlock(in_channels,in_channels)
    ...
    def forward(self, x):
        ...
        x = x.reshape((x.shape[0],x.shape[1], self.patch_h, self.patch_w))
...
class LGFI(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
        ...
        if (dim==48):
            self.MLP_mixer = MLPMixer(in_channels=48, image_size=[48,160], patch_size=4, dim=48, depth=1, token_dim=480, channel_dim=48)
            
            
        if (dim==80):
            self.MLP_mixer = MLPMixer(in_channels=80, image_size=[24,80], patch_size=4,  dim=80, depth=1, token_dim=120, channel_dim=160)
            
            
        if (dim==128):
            self.MLP_mixer = MLPMixer(in_channels=128, image_size=[12,40], patch_size=4,  dim=128, depth=1, token_dim=30, channel_dim=128)            
